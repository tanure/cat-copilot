...
/**
 * Stable (non-partitioned) domain directory names. These knowledge/project/
 * learning/achievement trees live directly under storage.root — NOT inside a
 * day/week/month partition — so folders + tags map cleanly to an Obsidian graph
 * and cross-item aggregation is a single tree walk.
 */
...
/**
 * Resolve a stable, non-partitioned domain directory under storage.root.
 * @param {string} type - 'knowledge' | 'projects' | 'learning' | 'achievements'
 * @param {object} config - Config object (with __baseDir)
 * @param {string} projectRoot - Project root directory
 * @returns {string} Absolute path to the stable domain directory
 */
function resolveStableDir(type, config, projectRoot = process.cwd()) {
  const base = config.__baseDir || projectRoot;
  const storageRoot = path.resolve(base, config.storage.root);
  const dirName = (config.storage.files && config.storage.files[type]) || STABLE_DIR_DEFAULTS[type];
  if (!dirName) {
    throw new Error(`Unknown stable dir type: ${type}`);
  }
  return path.join(storageRoot, dirName);
}

/**
 * Recursively collect files/dirs named `targetName` under `root` (depth-capped,
 * skipping dotfolders). Used to read-merge legacy partitioned notes.
 * @param {string} root - Directory to search
 * @param {string} targetName - File or dir name to match
 * @param {boolean} isDir - Whether to match directories (true) or files (false)
 * @returns {string[]} Absolute paths of matches
 */
function walkFind(root, targetName, isDir, depth = 0, acc = []) {
...
function formatTasksTable(tasks) {
  ...
  // Section 1 holds every task that is NOT done (Open, Blocked, or any other
  // non-done status) so custom statuses survive the round-trip instead of being
  // silently dropped. Section 2 holds completed tasks.
  ...
  if (openTasks.length > 0) {
    content += '## Open Tasks\n\n';
    content += '| ID | Status | Title | Due Date | Priority | Tags | Context | Project |\n';
    content += '| --- | --- | --- | --- | --- | --- | --- | --- |\n';
    openTasks.forEach(t => {
      content += `| ${t.id} | ${t.status} | ${t.title} | ${t.dueDate} | ${t.priority} | ${t.tags} | ${t.context} | ${t.project || ''} |\n`;
    });
    content += '\n';
  }
